...
def get_numa_hardware():
    ...
    for line in r.stdout.splitlines():
        ...
        name = cp[0]
        if  name == "available":
            numa_node_count = int(cp[1])
            continue
        if name[-4:] == "cpus":
            cpulist.append(cp[1].strip())
            continue
        if name[-4:] == "size":
            for i in cp[1].strip().split():
                sizes.append(int(i))
            continue
        if name[-4:] == "nces":
            # distances
...
class Builder():
    ...
    def get_value(self, path_full: str) -> adc_types.Field:
        ...
        if isinstance(jit, float):
            field.st = adc_types.ScalarType.cp_double
            field.data = jit
        ...
        if isinstance(value, decimal.Decimal):
            field.st = adc_types.type_to_scalar(type(value), value)
            field.data = value
    ...
    def add_section(self, name: str,
        ...
        """
        if name in self._d:
            raise TypeError("attempting to replace a key/value with a section. Not Allowed. %s: %s",
                name, section)
    ...
    def add(self, name: str, value: any, value_type=None):
        ...
        elif isinstance(value, list|collections.abc.Set|tuple):
            depth = 0
            maxdepth=[0]
            ...
            if it != adc_types.ScalarType.cp_none:
                ...
                if maxdepth[0] == 1:
                    ...
                    self._set_attr(name, {"type": adc_types.scalar_to_json(it),
                        "value": list_value, "container_type": cname, "major": "row"})
                    return
            else:
